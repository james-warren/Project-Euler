...
for i in range( 0, (len(lst_of_products)) ):
    ...
    if ( i == 0):
        ...
        if ( (max(lst_of_products)) == (lst_of_products[i]) ):
            ...
            for item in numbers_for_product:
                highest_prod_numbers.append(item)
    ...
    # all other cases
        ...
        print("hpn2", highest_prod_numbers) #***this is where update  highest prod number error happens!!! - now fixed.
        ...
        if ( max(lst_of_products) == lst_of_products[i] ):
            ...
            print("highest prod numbers", highest_prod_numbers)
        #* WHY IS THIS ALWAYS UPDATING? - now fixed
...
# *BUG this always seems to update to the last product, not max!!! -now fixed
